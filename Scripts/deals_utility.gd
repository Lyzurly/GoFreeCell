...
static func start_random_deal() -> int:
	...
	match player_settings.difficulty:
		PlayerSettings.Difficulty.Any:
			deal_number = (randi() % player_settings.max_deal_number) + 1
		PlayerSettings.Difficulty.Easy:
			deal_number = easy_deals[randi() % easy_deals.size()]
		PlayerSettings.Difficulty.Hard:
			deal_number = hard_deals[randi() % hard_deals.size()]
	_deal(deal_number)
	Board.ref.on_cards_dealt(deal_number, false)

	return deal_number


static func start_custom_deal(deal_number: int):
	_deal(deal_number)
	Board.ref.on_cards_dealt(deal_number, false)


static func redeal():
	_deal(Board.ref.current_deal_number)
	Board.ref.on_cards_dealt(Board.ref.current_deal_number, Board.ref.is_challenge_deal)


static func _deal(deal_number: int):
	for cell in Board.ref.free_cells:
		cell.clear()
	for cell in Board.ref.foundation_cells:
		cell.clear()
	for cell in Board.ref.tableau_cells:
		cell.clear()
	...
	pseudo_rng.set_seed(deal_number)
	...
	while cards_left > 0:
		...
		Board.ref.tableau_cells[column].add_child(card)
		...
		var suit = card_nbr & 3
		card.init(suit, rank, Board.ref.tableau_cells[column])

		column = (column + 1) % Board.ref.tableau_cells.size()
		cards_left -= 1	
